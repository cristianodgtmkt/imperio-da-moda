...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { preferred_sizes, preferred_categories, preferred_colors, preferred_occasions, notes } = await req.json();

  const [profile] = await query(
    `INSERT INTO customer_profiles (customer_id, preferred_sizes, preferred_categories, preferred_colors, preferred_occasions, notes, updated_at)
     ...
     ON CONFLICT (customer_id) DO UPDATE SET
       ...
       updated_at = now()
     RETURNING *`,
    [params.id, preferred_sizes ?? [], preferred_categories ?? [], preferred_colors ?? [], preferred_occasions ?? [], notes ?? '']
  );
